...
const expected = new Map([
  ['opposite-upstairs-1', ['1 single bed', 'opposite-upstairs-1']],
  ['opposite-upstairs-2', ['2 single beds', 'opposite-upstairs-2']],
  ...
  ['castle-upstairs-3', ['2 single beds', 'castle-upstairs-3']],
  ['castle-upstairs-4', ['3 single beds', 'castle-upstairs-4']],
  ['castle-upstairs-5', ['1 double bed', 'castle-upstairs-5']],
  ['castle-upstairs-6', ['1 double bed', 'castle-upstairs-6']],
  ['opposite-right-upstairs-new', ['4 single beds', null]],
]);
...
function sourceChecks() {
  ...
  assert.equal(allRooms.length, 18);
  assert.equal(allRooms.filter((room) => room.slug).length, 17);
  assert.equal(allRooms.filter((room) => !room.slug).length, 1);
  assert.match(page, /robots:\s*"noindex, nofollow, noarchive, nosnippet"/);
  assert.match(page, /sitemap:\s*\n\s+disable: true/);
  assert.match(page, /unchanged photos already used on the Open Blues website/i);
...
async function browserChecks() {
  ...
  try {
    for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
      ...
      assert.equal(await page.locator('[data-inventory-room]').count(), 18);
      assert.equal(await page.locator('[data-inventory-room] img').count(), 17);
      assert.equal(await page.locator('.inventory-photo-missing').count(), 1);
...
async function run() {
  sourceChecks();
  await browserChecks();
  process.stdout.write('PASS: unlinked room inventory check reuses 17 canonical room photos, preserves all corrected labels, and stays out of navigation and sitemap.\n');
}
