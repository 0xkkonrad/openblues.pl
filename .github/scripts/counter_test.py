...
Covers the two things the counter must never get wrong:
  ...
  2. it must fail LOUDLY and write nothing when either workbook guard trips, so the site keeps
     serving the last good counter instead of a confident wrong number.
"""
import json
import os
import subprocess
...
HEALTHY = "formulas OK - status is automatic | FEED OK - counting from 'Form Responses 1'"
FAILURES = []
CHECKS = []
...
rc, out, log = run(HDR + f"\n3,40,open,2027-01-02 10:00 UTC,,,{HEALTHY},\n")
...
rc, out, log = run(HDR + f"\n41,40,confirmed,2027-04-11 19:03 UTC,,,{HEALTHY},2027-04-11 19:03 UTC\n")
check("exit 0", rc == 0, "rc=%d" % rc)
...
rc, out, log = run(HDR + f"\n39,40,open,2027-04-12 08:00 UTC,,,{HEALTHY},\n", previous=prev)
check("exit 0", rc == 0, "rc=%d" % rc)
...
rc, out, log = run(
    HDR + f"\n39,40,cancelled,2027-08-04 09:00 UTC,,,formulas OK - MANUAL OVERRIDE | FEED OK,\n",
    previous=prev,
)
check("exit 0", rc == 0, "rc=%d" % rc)
...
rc, out, log = run(HDR + f"\n40,40,confirmed,2027-04-11 19:03 UTC,,,{HEALTHY},2027-04-11 19:03 UTC\n",
                   previous=prev_open)
check("status becomes confirmed", out and out["status"] == "confirmed", json.dumps(out))

print("\n== no previous counter.json at all ==")
rc, out, log = run(HDR + f"\n0,40,open,2027-01-01 00:00 UTC,,,{HEALTHY},\n")
check("exit 0 and publishes open", rc == 0 and out and out["status"] == "open", json.dumps(out))

print("\n== FAIL CLOSED: the raw form feed is missing but paid is still numeric ==")
rc, out, log = run(
    HDR + '\n0,40,open,2027-01-01 00:00 UTC,,,"formulas OK | FEED MISSING - check source tab",\n',
    previous=prev,
)
check("exit non-zero on FEED MISSING", rc != 0, "rc=%d" % rc)
check("FEED MISSING leaves counter.json alone", out == prev, json.dumps(out))
check("the error identifies the feed guard", "feed ok" in log.lower(), log.strip()[-120:])

print("\n== FAIL CLOSED: one healthy token is not enough ==")
for label, guard_text, missing in [
    ("formulas only", "formulas OK", "feed ok"),
    ("feed only", "FEED OK", "formulas ok"),
    ("missing check column", "", "formulas ok"),
]:
    if label == "missing check column":
        csv_text = "paid,threshold,status,updated\n7,40,open,x\n"
    else:
        csv_text = HDR + f"\n7,40,open,x,,,{guard_text},\n"
    rc, out, log = run(csv_text, previous=prev)
    check(f"{label}: exits non-zero", rc != 0, "rc=%d" % rc)
    check(f"{label}: preserves previous counter", out == prev, json.dumps(out))
    check(f"{label}: names missing guard", missing in log.lower(), log.strip()[-120:])
...
check("counter.json is LEFT ALONE (site keeps the last good counter)", out == prev,
      json.dumps(out))
check("the error names the offending value", "HEADER MISMATCH" in log, log.strip()[-90:])
