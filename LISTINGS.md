# Open Blues — Festival Listings Tracker

**2027 status:** every listing below was made for the 2026 edition and must be re-submitted or edited for 2027 (dates 26–30 Aug 2027, assumed — confirm against `hugo.toml`). 2027 is by application: link to https://openblues.pl until the application form is live, then to the form. The log at the bottom is the 2026 history.

Where Open Blues (26–30 Aug 2027, Piotrowice Nyskie Palace, Poland) should be listed to reach the blues, swing, and fusion dance crowds. Scope is deliberately narrow: blues / swing (lindy, balboa, shag-adjacent) / fusion only — no generic event sites, no other dance scenes.

Research verified 2026-07-12 by live-fetching every site (deep-research run, 25/25 claims confirmed by 3-0 adversarial votes). Dance calendars go stale fast: re-check a submission page right before submitting if more than a few weeks have passed.

Status legend: `TODO` not started · `SUBMITTED` waiting on moderation · `LIVE` listing visible · `VERIFY` already listed by someone, needs checking · `BLOCKED` needs something first · `SKIP` decided against.

## Status board

| # | Venue | Scene | Route | Cost | Status | Next action |
|---|-------|-------|-------|------|--------|-------------|
| 1 | [BluesCal.com](https://bluescal.com/) | Blues (global) | Self-serve form, no account | Free | SUBMITTED | Watch calendar for the listing to appear |
| 2 | [FusionCal.com](https://fusioncal.com/) | Fusion (global) | Same shared form | Free | SUBMITTED | Covered by #1 submission (Fusion ticked) |
| 3 | [DanceCal.com](https://dancecal.com/) | Blues/Lindy/Balboa/Solo Jazz/Fusion umbrella | Same shared form | Free | SUBMITTED | Covered by #1 submission |
| 4 | [SwingPlanIt](https://www.swingplanit.com/) | Swing worldwide, Blues category | Self-serve form, math captcha | Free | SUBMITTED | Weekly publish cycle: check around 19 Jul |
| 5 | [swingfestivals.com](https://www.swingfestivals.com/) | Swing worldwide, Blues filter | Email hello@swingfestivals.com | Free | SUBMITTED | Emailed 12 Jul; watch openbluespoland inbox |
| 6 | [Swing Calendar](https://swingcalendar.com/en/) | Swing worldwide, Blues subtype | Account (Organizer role) + form | Free | SUBMITTED | "Waiting for approval" on My Events; watch inbox |
| 7 | [fusion-dancing.eu](https://fusion-dancing.eu/) | Fusion (Europe) | Discord / GitHub / email | Free | SUBMITTED | Emailed events@ 12 Jul; watch inbox |
| 8 | [WhereCanWeDance](https://www.wherecanwedance.com/events/open-blues) | Multi-style (Blues+Fusion tags) | Account-gated | Free | LIVE | Fill registration/social URLs: needs login to owner account 1444 |
| 9 | [Blues in Europe Calendar](https://www.annettedances.com/blues-in-europe-calendar/) | Blues (Europe, curated) | Message the maintainer | Free | LIVE | None: entry verified correct, links openblues.pl |
| 10 | [Swing Wings calendar](https://www.swingwings.cz/calendar/) | Czech swing+blues (Brno) | Email marek@swingwings.cz | Free | SUBMITTED | Emailed 12 Jul; watch inbox |
| 11 | Blues Dancing in Poland (FB group) | Polish blues | FB account + join group | Free | BLOCKED | Konrad: join + post (PL copy below) |
| 12 | Blues Dance in Krakow / KOLIBR (FB group) | Kraków blues | FB account + join group | Free | BLOCKED | Konrad: join + post (PL copy below) |
| 13 | Krakow Fusion Dance Community (FB group) | Kraków fusion | FB account + join group | Free | BLOCKED | Konrad: join + post (PL or EN copy below) |
| 14 | Fusion Dance Europe (FB group) | Pan-European fusion | FB account + join group | Free | BLOCKED | Konrad: join + post (EN copy below) |
| 15 | European Fusion Dancing Discord | European fusion | Join Discord, post event | Free | BLOCKED | Konrad: post (copy below); calendar itself already covered by #7 email |
| 16 | Lindy Hop in Cracow (FB group) | Kraków swing | FB account + join group | Free | BLOCKED | Konrad: check group promo rules, then post |
| 17 | Fusion Dance Vienna (FB group) | Vienna fusion | FB account + join group | Free | BLOCKED | Lowest priority; existence confirmed, activity not |
| 18 | Kraków Blues Happenings (FB page) | Kraków blues | Message the page | Free | BLOCKED | Konrad: message the page asking for a share |

## Submission data pack

Copy-paste facts for every form. Messaging policy applies everywhere: Open Blues is cost-sharing, not a product. Never write "price", "ticket price", or discount language in a listing. If a form has a price field, leave it empty or write "cost-sharing"; the description below deliberately omits numbers (SwingPlanIt bans prices in descriptions anyway) and lets openblues.pl explain the model.

- **Event name:** Open Blues 2027
- **Dates:** 26–30 August 2027 (Thu–Mon) — assumed; must match `eventDatesHuman` in `hugo.toml`
- **Location:** Piotrowice Nyskie Palace, Piotrowice Nyskie 54 (near Nysa), Silesia, Poland. Nearest airports Wrocław, Ostrava, Katowice, Pardubice, Brno; nearest rail hub Nysa.
- **Country:** Poland · **City field:** Piotrowice Nyskie (add "near Wrocław/Nysa" where a free-text field allows)
- **Styles:** Blues + Fusion (where Fusion isn't offered, e.g. SwingPlanIt: Blues)
- **Event type:** Exchange (DanceCal-family taxonomy) / Camp (SwingPlanIt taxonomy — residential, food and beds on site)
- **Website:** https://openblues.pl
- **Application:** https://openblues.pl (the application form link is added there once applications open; 2027 is by application, everyone hears back by email)
- **Contact email:** openbluespoland@gmail.com
- **Teachers field:** "Community-led — workshops are crowdsourced from attendees, plus guest teachers" (no fixed lineup; SwingPlanIt forbids teacher names inside the description, use its separate field)
- **Facebook event:** none — several venues like linking one; decide whether to create one before the FB-group round (#11–14, 16–17)

**Description (short, ~55 words):**
> Open Blues is a non-profit, do-it-yourself blues and fusion dance festival in a historic palace in south-west Poland. Five days of crowdsourced workshops, live music, social dancing until sunrise, and shared cooking. No audience segregation by level — everyone teaches, everyone learns. Attendees share the festival's real costs. All scenes welcome.

Rule reminder for SwingPlanIt's description field: no web links, no teacher names, no prices, no venue address inside the description (they have separate fields, and incomplete or rule-breaking submissions don't go live).

## Tier 1 — Core registries (verified live, July 2026)

### 1–3. The DanceCal family: BluesCal + FusionCal + DanceCal (one submission)

- **What:** Dave Madison's calendar network — the de-facto global registries for the target scenes. BluesCal is the primary dedicated blues event calendar; FusionCal the dedicated fusion one; DanceCal the umbrella (filters: Blues, Lindy, Balboa, Solo Jazz, Fusion — exactly our scope, no salsa/tango/ballroom).
- **Reach:** Worldwide with dense Europe coverage. BluesCal has European events through Oct–Nov 2026 (Hasselt, Brno, Bern, Porto, Lithuania) and currently **zero Poland events** — we'd be the only one. FusionCal lists Berlin, Turin, Barcelona, Bruges, Newcastle into 2027. DanceCal lists Herräng and Kraków events.
- **How:** Self-serve form at [bluescal.com/Submit/](https://bluescal.com/Submit/) (or /Submit/ on any of the three — the form header says "Submit a new event for DanceCal/BluesCal/FusionCal/BalCal", so **one submission feeds all of them** via the Dances checkboxes). No account needed. Moderator approves before it appears; "Submitting multiple times will not help!"
- **Fields:** Event Name, City, Country, Date, Type (Workshop / Exchange / House Party → use Exchange), Dances checkboxes (tick Blues + Fusion), Email. URL / Facebook / description optional.
- **Eligibility:** "Local events, non-social-dancing events and one-day events do not belong here and will not get approved." A 5-day international festival qualifies comfortably.
- **Cost:** Free (no fee mentioned anywhere).

### 4. SwingPlanIt — swingplanit.com

- **What:** The most comprehensive worldwide swing festival directory; the first place travelling swing dancers look.
- **Reach:** Global. Blues is a first-class submittable style (alongside Balboa, Boogie Woogie, Charleston, Jazz, Lindy Hop, Shag) and is actively used: 2026 European blues listings include Mainhatten Blues (Frankfurt), Blues Remix (Brussels), Paris Midnight Blues, Boulevard Blues (Hasselt), Blues Dose Retreat (CZ). No Fusion category — list under Blues.
- **How:** Free self-serve form at [swingplanit.com/index.php/swing-event-submission](https://www.swingplanit.com/index.php/swing-event-submission). No account; there's a small math captcha.
- **Fields:** name, dates, website URL, country, town, contact, event type (Camp / Weekender / Exchange only — no weekly socials; use Camp), description, teachers, style checkboxes.
- **Constraints:** description must NOT contain web links, teacher names, booking prices, or venue addresses (separate fields exist). Incomplete submissions are silently not published.
- **Lead time:** site updates once a week — allow 7 days; resubmitting doesn't speed it up. Submit early.
- **Cost:** Free.

### 5. swingfestivals.com

- **What:** Worldwide swing festival directory, 240+ festivals across 38 countries, listings May 2026 → April 2027.
- **Reach:** Global; Blues is a first-class style filter with real 2026 blues listings (Slow Flow Madrid, The Slow Down Brno, Summertime Blues Herchen, swingomania Nuremberg).
- **How:** **Email only** — there is no submission form, and the footer /contact link 404s (as do /submit, /add-event, /list-your-festival). The working route is the mailto CTA on [/about](https://www.swingfestivals.com/about): **hello@swingfestivals.com** ("If you organise a swing festival... get in touch. It's free and reaches the whole community.").
- **Cost:** Free (explicitly stated).
- **Note:** actively maintained (forward calendar to Apr 2027) though their blog is stale — judge by the calendar, not the blog.

### 6. Swing Calendar — swingcalendar.com

- **What:** Worldwide swing event directory, actively developed (platform release notes as recent as 2026-06-29), ad-tracker free.
- **Reach:** ~250–400 event pages with dates from Jul 2026 into 2027–28; genuine blues and Central-Europe coverage: Berlin Blues & Fusion Experience (Oct 2026), Prague Spring Swing Festival, Blues Workshop Weekend Magdeburg, Mainhatten Blues Frankfurt — 17 blues/fusion-named events total. Blues is a first-class subtype.
- **How:** Self-serve but **account-gated**: register a user account with the "Organizer" role, then + Create → fill details, upload photos, submit. Moderated review with email confirmation before publication.
- **Cost:** Free (explicit). Paid upsells (featured placement, ticketing) exist but don't gate a basic listing — ignore them.

### 7. fusion-dancing.eu

- **What:** THE Europe-specific fusion dance calendar, and the most Central-Europe-relevant venue found: it maps ~40 local scenes including Kraków, Warszawa, Praha, Wien, Berlin. ICS/WebCal/RSS feeds extend its reach.
- **How (three routes, all free):**
  1. **Discord** (preferred): the European Fusion Dancing Discord — `discord.gg/28ej3V2vQG` — events added there are auto-proposed to the site (pipeline confirmed live via automated GitHub commits).
  2. GitHub issue/PR on `fusion-dancing-eu/fusion-dancing-eu.github.io`.
  3. Email `events[at]fusion-dancing.eu`.
- **Eligibility (the one real hurdle):** events must be multi-day AND "dedicate at least roughly half their resources to Fusion dancing" — single-evening socials get pointed to local scene pages. Open Blues qualifies on duration; the fusion share is honestly fuzzy because the program is crowdsourced. Submitted 12 Jul via the email route; whether it meets their bar is now the moderators' call.
- **Maintained:** merged event PRs dated 2026-07-05 to -08; calendar lists 2026–27 events including Berlin Blues & Fusion Experience.

### 8. WhereCanWeDance — wherecanwedance.com ⚠️ already listed

- **What:** Global multi-style directory with dedicated Blues and Fusion calendars (plus out-of-scope styles).
- **State:** Open Blues is **already listed** at [wherecanwedance.com/events/open-blues](https://www.wherecanwedance.com/events/open-blues) (event id 5400). Re-verified 12 Jul: live and accurate (dates 2026-08-27→31, address Piotrowice Nyskie 54, styles Blues + Fusion, tags Festival + Social, links to openblues.pl). Created by user account **1444** (Organizer role) — **find out whether that's us** (a co-organizer?) or a stranger; if unknown, contact the site to claim it.
- **Remaining work:** fill the empty fields — ticket_url, facebook_url, instagram_url, meetup_url, tiktok_url are all null. Set ticket_url to the registration form. Blocked on logging in as account 1444.
- **How (new events generally):** "Add new event" at /suggest_event, free login required.
- **Caveat:** their Blues calendar is thin (only 2 upcoming events at check time), so blues-scene reach here is limited — low effort anyway since the listing exists.

## Tier 2 — Curated scene calendars (email/moderator route)

### 9. Blues in Europe Calendar (annettedances.com) ⚠️ already listed

- **What:** Community member Annette's curated calendar of ALL weekend-long-or-longer blues dance events in Europe & UK, explicitly including Blues & Fusion events. Best geographic fit of anything found; dense listings through Dec 2026 into 2027, covering Vienna, Berlin, Brno and Poland.
- **State:** it **already lists "Open Blues & Fusion near Wrocław, August 27–31"**. Re-verified 12 Jul: the entry links to openblues.pl with correct dates. No action needed.
- **How:** no form — "send me a quick message if you're missing your (or someone else's) event" via the site's contact link. Free, informal.

### 10. Swing Wings calendar (swingwings.cz)

- **What:** The Czech (Brno-centred) swing+blues scene calendar; Czech-language, subscribe-able (iCal), links out to festivals. Lists multi-day festivals, not just socials (e.g. Sem Tam Blues, 7–9 Aug 2026).
- **Why:** Brno is squarely in our catchment (it's on our own airports list), and the Czech blues crowd is the nearest foreign scene.
- **How:** no form — email **marek@swingwings.cz**. Free. Verified live (events through Aug 2026).

## Tier 3 — Community channels (Facebook / Discord)

Existence of all these groups is confirmed; their activity levels, member counts, and posting rules are NOT verifiable without a logged-in Facebook account — the research agents hit login walls. Someone with a Facebook account (Konrad or a co-organizer) needs to join and post; draft post copy is in the "Ready-to-paste copy" section below. A Facebook event page would help in all of them; decide whether to create one first. Free everywhere.

| Channel | Link | Why |
|---------|------|-----|
| Blues Dancing in Poland | facebook.com/groups/bluesdancinginpoland | The national Polish blues group — the single most direct channel to Polish blues dancers for a Silesia festival |
| Blues Dance in Krakow (KOLIBR) | facebook.com/groups/376240249215885 | Kraków's blues community (KOLIBR scene org) — nearest large Polish blues scene |
| Krakow Fusion Dance Community | facebook.com/groups/krakowfusiondance | Kraków fusion scene; public group; companion page facebook.com/krakow.fusion.dance as a second admin contact |
| Fusion Dance Europe | facebook.com/groups/669350926865075 | The pan-European fusion group — fusion's de-facto announcement board since the scene has no dominant commercial directory |
| European Fusion Dancing Discord | discord.gg/28ej3V2vQG | Pan-European fusion chat; posting an event here ALSO auto-proposes it to fusion-dancing.eu (#7) — do this one first |
| Lindy Hop in Cracow | facebook.com/groups/lindyhopkrakow | Kraków swing scene (KMiTa Swing orbit) — swing dancers are the main crossover pool; check group rules on event promo |
| Fusion Dance Vienna | facebook.com/groups/941986849993191 | Vienna is a named catchment scene; group existence confirmed but activity unverified — lowest priority |
| Kraków Blues Happenings | facebook.com/BluesHappenings | Page (not group) around Kraków's blues weekend — ask for a share; goodwill-based, no form |

Not covered by this research (verified gap, needs a follow-up pass if we care): Wrocław and Prague scene groups, Berlin blues (as opposed to fusion/swing) channels, and any scene newsletters. No blues/swing/fusion newsletter that functions as an event registry survived verification.

## Do not submit / does not exist

- **swinghopping.com** — formerly popular swing/blues calendar, **verified dead**: 301-redirects to an unrelated site (savoirchanger.org). Still surfaces in search results and old link lists; don't waste time.
- **bluesdance.world / "Blues Dance World"** — appears not to exist (was a search anchor; nothing found). BluesCal fills this role.
- **thedanceatlas.com** — Europe+Blues aggregator view exists (~70–190 events, maintained), but submission mechanics are unverified and it may be scraped rather than submitted-to. Revisit only if the Tier 1 round leaves gaps.
- **danceus.org** — US-skewed generalist dance platform with a swing/blues festival directory; account needed and the footer mentions subscription plans. Out of scope per the "no generic listing sites" rule.
- **swing.fandom.com (Lindypedia)** — wiki of bare event names, effectively stale; no meaningful reach.

## Open questions

1. **WhereCanWeDance account 1444** — who created our listing? If not us, how do we claim/edit it? This is the only blocker for filling the empty registration/social fields.
2. **Facebook event page** — create one (helps every Tier 3 channel) or stay web-only? The draft copy below works for either.
3. **Tier 3 posting** — gated on Konrad's personal Facebook and Discord accounts.

~~fusion-dancing.eu fusion-share pitch~~ — resolved by submitting via email on 12 Jul; their moderators decide.

## Ready-to-paste copy (drafts)

Written to the messaging policy: cost-sharing, never "price"/"ticket". Passed the subagent AI-tell cold read on 12 Jul (fixes applied; "na Opolszczyźnie" geography verified). The Facebook headlines are plain text on purpose: Facebook doesn't render Markdown bold, so asterisks would paste literally. Adjust freely.

### English — Facebook groups (#14, #13, #16, #17) and the event page description

> Open Blues 2027: DIY blues & fusion festival in a Polish palace, 26–30 August
>
> Open Blues is back: five days at Piotrowice Nyskie Palace in south-west Poland, an hour from Wrocław and about three hours from Kraków, Prague and Vienna.
>
> It's a non-profit, do-it-yourself festival. We handle the venue and logistics; the workshops, live music and cooking come from everyone who shows up. There's social dancing until sunrise in the half-renovated palace ballroom, and we cook and eat together. Classes are taught by whoever comes, plus a few guest teachers. If there's something you love dancing or teaching, bring it and run a session.
>
> Instead of a ticket, everyone shares the festival's real costs. A full Thursday–Monday stay starts at €130; a mattress place with average drinks is €205, including the €50 reservation.
>
> Info and application: https://openblues.pl
> Questions: openbluespoland@gmail.com

### Polish — Facebook groups (#11, #12, #13)

> Open Blues 2027: oddolny festiwal bluesa i fusion w pałacu, 26–30 sierpnia
>
> Open Blues wraca: pięć dni w pałacu w Piotrowicach Nyskich na Opolszczyźnie, godzinę drogi od Wrocławia.
>
> To niekomercyjny festiwal DIY. Jako organizatorzy ogarniamy pałac i logistykę, a warsztaty, muzykę na żywo i gotowanie robią sami uczestnicy. Tańczymy do wschodu słońca w na wpół wyremontowanej sali balowej, gotujemy i jemy razem, a zajęcia prowadzą uczestnicy i zaproszeni nauczyciele. Jest coś, co kochasz tańczyć albo czego chcesz uczyć? Poprowadź z tego warsztat.
>
> Zamiast biletu dzielimy się realnymi kosztami. Pełny pobyt od czwartku do poniedziałku zaczyna się od 130 €; miejsce na materacu ze średnim pakietem napojów kosztuje 205 €, wliczając rezerwację 50 €.
>
> Info i zgłoszenia: https://openblues.pl
> Pytania: openbluespoland@gmail.com

### Discord — European Fusion Dancing (#15), short

> Hi! We're organising **Open Blues 2027**, a 5-day DIY blues & fusion festival at Piotrowice Nyskie Palace in Poland, 26–30 Aug. Attendee-taught workshops, live music, dancing till sunrise. Cost-sharing instead of tickets; a full stay starts at €130. https://openblues.pl
> Happy to answer questions! (Mods: calendar listing already sent to events@fusion-dancing.eu.)

## Log

| Date | Venue | Action | Result |
|------|-------|--------|--------|
| 2026-07-12 | (all) | Deep research: verified 8 registries live, 2 pre-existing listings found, 1 dead site flagged | Tracker created |
| 2026-07-12 | DanceCal family (#1–3) | Submitted via shared bluescal.com/Submit/ form: Blues + Fusion, Exchange, Poland | Awaiting moderation |
| 2026-07-12 | SwingPlanIt (#4) | Submitted via event form: Camp, Blues (no Fusion in taxonomy) | Awaiting weekly publish (~19 Jul) |
| 2026-07-12 | swingfestivals.com (#5) | Emailed hello@swingfestivals.com from openbluespoland@gmail.com, signed Konrad | Awaiting reply |
| 2026-07-12 | Swing Calendar (#6) | Account created, event submitted | "Waiting for approval" on My Events; confirmation email in inbox |
| 2026-07-12 | fusion-dancing.eu (#7) | Emailed events@fusion-dancing.eu, signed Konrad | Awaiting reply |
| 2026-07-12 | Swing Wings (#10) | Emailed marek@swingwings.cz, signed Konrad | Awaiting reply |
| 2026-07-12 | WhereCanWeDance (#8) | Re-verified existing listing | Live and accurate; field-fill blocked on account 1444 |
| 2026-07-12 | annettedances (#9) | Re-verified existing entry | Live, correct dates, links openblues.pl; done |
| 2026-07-12 | (drafts) | Subagent AI-tell cold read of FB/Discord copy | Fixes applied: first-person voice, PL calques removed, FB markdown stripped, city overclaim cut |
